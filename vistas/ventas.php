...
$(document).ready(function() {
            ...
            success: function(respuesta) {
                ...
                if (parseInt(respuesta['existe']) == 0) {
                    ...
                    NuevoPrecio = (parseInt(data['cantidad']) * data[
                        'precio_venta_producto'].replace("S./ ", "")).toFixed(2);
    ...
    $('#lstProductosVenta tbody').on('click', '.btnDisminuirCantidad', function() {
        ...
        if (data['cantidad'].replace('Und(s)', '') >= 2) {
            ...
            NuevoPrecio = (parseInt(data['cantidad']) * data['precio_venta_producto'].replace("S./ ",
                "")).toFixed(2);
            NuevoPrecio = "S./ " + NuevoPrecio;
...
/*actualiza el campo vuelto*/

function actualizarVuelto() {
    var totalVenta = $("#totalVenta").html();
    $("#chkEfectivoExacto").prop('cheked', false);
...
function recalcularMontos(codigo_producto, precio_venta) {
    ...
    table.rows().eq(0).each(function(index) {
        ...
        if (data['codigo_producto'] == codigo_producto) {
            ...
            NuevoPrecio = (parseFloat(data['cantidad']) * data['precio_venta_producto'].replaceAll("S./ ", ""))
                .toFixed(2);
            NuevoPrecio = "S./ " + NuevoPrecio;
...
function CargarProductos(producto = "") {
    ...
    //aumentar cantidad si el producto existe en listado
    table.rows().eq(0).each(function(index) {
        var row = table.row(index);
        var data = row.data();
                ...
                if (respuesta['aplica_peso'] == 1) {
                    ...
                    table.row.add({
                        ...
                        'acciones': "<center>" +
                            ...
                            "' style='cursor:pointer; font-size:14px;'>Por Mayor (S./ " +
...
function realizarVenta() {
    ...
    var totalVenta = $("#totalVenta").html();
     ...
     var tipo_comprobante = $("#selDocumentoVenta").val();
            ...
            var formData = new FormData();
             ...
             table.rows().eq(0).each(function(index) {
                 ...
                 arr[index] = data['codigo_producto'] +',' +
                  parseFloat(data['cantidad']) + ',' + data['total'].replace("S/. ","");
